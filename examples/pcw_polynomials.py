...
axs[0].plot(t1,y1_pcw.get_value(t1),'*',c='black', label=f'value at {t1}', markersize=15)
axs[1].plot(t2,y2_pcw.get_value(t2),'*',c='black', label=f'value at {t2}', markersize=15)
axs[0].set_ylabel('pos1')
axs[1].set_ylabel('pos2')
axs[1].set_xlabel('Time(Second)')
axs[0].set_title(f'Piecewise Polynomial')
for ax in axs:
    ax.legend()
plt.show()
