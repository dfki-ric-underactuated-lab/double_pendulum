...
class runningmean_filter(AbstractFilter):
    def __init__(
        self,
        window=10,
    ...
    def save_(self, save_dir):
        ...
        with open(
            os.path.join(save_dir, "filter_runningmean_parameters.yml"), "w"
        ) as f:
    ...
    def get_filtered_state_(self, x, u, t=None):
        ...
        self.x_hist[-1] = x_filt
        x_filt = np.mean(self.x_hist[max(0, len(self.x_hist) - self.window) :], axis=0)
