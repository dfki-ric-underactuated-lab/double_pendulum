...
class CustomEnv(gym.Env):
    def __init__(
        ...
        terminated_func,
        reset_func,
        # define the states as (s,a,s') to fight the time delay of the real model
        ...
        self.observation_previous = None
        self.observation = self.reset_func()
        self.step_counter = 0

    def step(self, action):
        self.observation_previous = self.observation_current
        self.observation_current = self.dynamics_func(self.observation, action)
        # which order is this?
        self.obsercation = np.array(self.observation_previous,action,self.observation_current)
        reward = self.reward_func(self.observation_current, action)
        terminated = self.terminated_func(self.observation_current)
        info = {}
        truncated = False
        self.step_counter += 1
