...
def fast_fourier_transform(data_measured, data_desired, n, t):
    ...
    dml = [dm_pos, dm_vel, dm_tau]
    # dml is of type list and will later be reconverted to type dataframe

    dd_pos = data_desired["pos"]
    dd_vel = data_desired["vel"]
    dd_tau = data_desired["torque"]
    ddl = [dd_pos, dd_vel, dd_tau]
    # ddl is of type list and will later be reconverted to type dataframe

    # Compute the FFT
    dm_hat = np.fft.fft(dml, n)

    # Compute the power spectrum density for pos, vel, tau
    psd_measured = dm_hat * np.conj(dm_hat) / n

    # Use the PSD to filter out noise
    # frequencies with lower PSD than bounds get cut off
    bounds = [30, 100, 300]

    indices = np.empty_like(psd_measured)
    # find all freqs with large power
    for i in range(len(indices)):
        indices[i] = psd_measured[i] > bounds[i]
...
def scipy_fft(data_measured, smooth_freq=100):
    ...
    # cuts off frequencies that are much lower (/smooth_freq)
    # then the main frequency at spectrum.max.()
