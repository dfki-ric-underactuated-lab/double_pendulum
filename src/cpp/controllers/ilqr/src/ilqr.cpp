...
void ilqr::set_cost_parameters(double su1, double su2,
...
void ilqr::set_model_parameters(double m1, double m2,
                                double l1, double l2,
...
void ilqr::set_start(Eigen::Vector<double, n_x> x){
    x0 = x;
}

void ilqr::set_start(double pos1, double pos2,
...
void ilqr::set_x_init_traj(double p1[], double p2[],
                           double v1[], double v2[]){
    for(int i=0; i<N; i++){
        x_traj[i](0) = p1[i];
        x_traj[i](1) = p2[i];
...
Eigen::Vector<double, n_x> ilqr::discrete_dynamics(Eigen::Vector<double, n_x> x,
...
double ilqr::final_cost(Eigen::Vector<double, n_x> x){
    ...
    en_error = pow(plant.calculate_total_energy(x) - goal_energy, 2.);
    fcost = fCp1*pos1_error + fCp2*pos2_error + 
            fCv1*vel1_error + fCv2*vel2_error + 
            fCen*en_error;
    return fcost;
}
...
void ilqr::compute_final_x(Eigen::Vector<double, n_x> x){
    ...
    final_x(1) = 2.*fCp2*(std::fmod(x(1) + M_PI, 2.*M_PI) - M_PI - goal(1) + eps)
                 + 2.*fCen*en_diff*E_x(1);
    final_x(2) = 2.*fCv1*(x(2) - goal(2))
                 + 2.*fCen*en_diff*E_x(2);
...
void ilqr::calculate_V_terms(){
    ...
    V_x = Q_x + K.transpose()*Q_u + 0.5*(Q_ux.transpose()*k + K.transpose()*(Q_uu.transpose()*k));
    V_xx = Q_xx + Q_ux.transpose()*K + K.transpose()*Q_ux + K.transpose()*Q_uu*K;
    if (verbose > 4){
...
void ilqr::forward_pass(){
    ...
    for (int i=0; i<N-1; i++){
        ...
        u_traj_new[i] = u_traj[i]  + k_traj[i] + K_traj[i]*(x_traj_new[i] - x_traj[i]);
...
Eigen::Vector<double, n_u>* ilqr::get_u_traj(){
    return u_traj;
}
